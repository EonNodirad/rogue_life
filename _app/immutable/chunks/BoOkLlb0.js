...
`,Sr="rogue_life",we="db",qn="save";function gn(){return new Promise((i,o)=>{const s=indexedDB.open(Sr,1);s.onupgradeneeded=()=>s.result.createObjectStore(we),s.onsuccess=()=>i(s.result),s.onerror=()=>o(s.error)})}async function yr(){const i=await gn();return new Promise((o,s)=>{const b=i.transaction(we,"readonly").objectStore(we).get(qn);b.onsuccess=()=>o(b.result??null),b.onerror=()=>s(b.error)})}async function Ir(i){const o=await gn();return new Promise((s,d)=>{const b=o.transaction(we,"readwrite");b.objectStore(we).put(i,qn),b.oncomplete=()=>s(),b.onerror=()=>d(b.error)})}let Z=null;async function v(){var s;if(Z)return Z;(s=navigator.storage)!=null&&s.persist&&await navigator.storage.persist();const i=await Lr({locateFile:()=>`${gr}/sql-wasm.wasm`}),o=await yr();return o?Z=new i.Database(o):(Z=new i.Database,Z.run(wr),await I()),Z}async function I(){Z&&await Ir(Z.export())}function Tn(i){return i.replace(/\$\d+/g,"?")}function _(i,o=[]){const s=Z.prepare(Tn(i)),d=[];for(s.bind(o);s.step();)d.push(s.getAsObject());return s.free(),d}function E(i,o=[]){Z.run(Tn(i),o)}const Se={1:{xp:15,gold:8,pv:5},2:{xp:30,gold:15,pv:10},3:{xp:50,gold:25,pv:20}};function ht(i){return i==="cauchemar"?2:i==="hard"?1.5:1}const Rn={pv_combat_max:100,attq:50,attq_spe:50,def:50,def_spe:50,vitesse:50,mana_max:100};async function Ur(i){return await v(),_("SELECT * FROM personnage WHERE id = $1",[i])[0]??null}async function Or(i){return await v(),_("SELECT * FROM caracteristique WHERE id = $1",[i])[0]??null}async function Ar(){return await v(),_("SELECT * FROM level ORDER BY id")}async function Mr(){return await v(),_("SELECT * FROM titre")}async function Fr(i,o){await v(),E("UPDATE personnage SET titre_id = $1 WHERE id = $2",[o,i]),await I()}async function Cr(){return await v(),_("SELECT * FROM stuff")}async function Dr(i){return await v(),_("SELECT * FROM inventaire WHERE personnage_id = $1",[i])}const Gr=["arme_1main","arme_2mains","bouclier_1main","bouclier_2mains"],_t=i=>i.includes("2mains")?2:1;async function Pr(i,o){var b;await v();const d=(b=_("SELECT s.slot FROM inventaire i JOIN stuff s ON s.id = i.stuff_id WHERE i.id = $1",[o])[0])==null?void 0:b.slot;if(d){if(Gr.includes(d)){const T=_t(d),L=_(`SELECT i.id, s.slot FROM inventaire i
         ...
         JOIN competence c ON c.id = pc.competence_id
         WHERE pc.personnage_id = $1 GROUP BY c.element`,[i]),T={feu:14,eau:15,terre:16,air:17,technologie:18,surnaturel:19,mort:20,vie:21,lumiere:22,tenebres:23},L={feu:4,eau:5,terre:6,air:7,technologie:8,surnaturel:9,mort:10,vie:11,lumiere:12,tenebres:13};for(const g of b)if((g.cnt??0)>=10&&T[g.element]){await Y(o,T[g.element]);return}for(const g of b)if((g.cnt??0)>=5&&L[g.element]){await Y(o,L[g.element]);return}if(o.level_id>=5){await Y(o,3);return}if(o.level_id>=3){await Y(o,2);return}await Y(o,1)}async function qa(i){var j;await v();const o=_("SELECT * FROM personnage WHERE id = $1",[i])[0];if(!o)return[1];const s=[1];o.level_id>=3&&s.push(2),o.level_id>=5&&s.push(3),o.gold_actuel>=5e3&&s.push(29);const{actuel:d,record:b}=await qt(i),T=Math.max(d,b);T>=7&&s.push(24),T>=14&&s.push(25),T>=30&&s.push(26),T>=180&&s.push(27),T>=365&&s.push(28);const L=_(`SELECT c.element, COUNT(*) as cnt FROM personnage_competence pc
         JOIN competence c ON c.id = pc.competence_id
         ...
         JOIN competence c ON c.id = pc.competence_id
         WHERE pc.personnage_id = $1 AND c.rarete = 'legendaire'`,[i])[0])==null?void 0:j.cnt)??0)>=3&&s.push(30),s}async function Y(i,o){i.titre_id!==o&&(E("UPDATE personnage SET titre_id = $1 WHERE id = $2",[o,i.id]),await I())}async function ga(i,o=[]){return await v(),_(i,o)}async function Ta(i,o=[]){await v(),E(i,o),await I()}const Sa=Object.freeze(Object.defineProperty({__proto__:null,ROUTINE_STATS:Se,STAT_SHOP_PRIX:Rn,_classesRun:Ta,_classesSelect:ga,acheterCompetence:aa,acheterItem:Wr,acheterStat:Xr,ajouterRecompenseDonjon:oa,allouerStat:ma,calculerEtAttribuerTitre:ha,calculerStreak:qt,calculerTitresDebloques:qa,changerMode:fa,changerTitre:Fr,checkDailyPenalties:Qr,checkLevelUp:la,checkModeCoffres:Ea,completerRoutine:Yr,completerTache:Jr,createTache:Vr,desequiperCompetence:ua,desequiperItem:xr,echouerTache:Zr,equiperCompetence:ia,equiperItem:Pr,gameOver:ye,getCaracteristique:Or,getCompetences:ta,getCompetencesDonjon:na,getHistoriqueActivite:ea,getInventaire:Dr,getLevels:Ar,getMagasinInventaire:Hr,getModeMultiplier:ht,getPersonnage:Ur,getPersonnageAffinites:pa,getPersonnageCompetences:ra,getStatsResume:_a,getStuffs:Cr,getTaches:jr,getTitres:Mr,incrementerLootDonjon:da,regenMana:Kr,renommerPersonnage:ba,resetPvCombat:sa,supprimerRoutine:kr,updateManaActuels:zr,updatePvCombat:ca,utiliserConsommable:$r,vendreItem:Br},Symbol.toStringTag,{value:"Module"}));export{Hr as A,ta as B,aa as C,Wr as D,_a as E,ea as F,Ea as G,jr as H,na as I,Jr as J,la as K,Zr as L,kr as M,Yr as N,oa as O,Vr as P,da as Q,Se as R,Rn as S,ca as T,zr as U,Kr as V,sa as W,Qr as X,Sa as Y,ga as _,Ta as a,Mr as b,ha as c,qa as d,qt as e,Or as f,Ur as g,Ar as h,Dr as i,Cr as j,pa as k,Fr as l,Xr as m,fa as n,ye as o,ma as p,ra as q,ba as r,wa as s,La as t,$r as u,xr as v,Pr as w,Br as x,ua as y,ia as z};
